...
function RecipeTablemanager() {
  ...
  const handleSaveChanges = async (updatedRecipe: createRecipe) => {
    try {
      ...
      await axios.put(`http://localhost:8080/api/recipes/update/${updatedRecipe.id}`, {
        id: updatedRecipe.id,
  ...
  const renderStatus = (status: Estate) =>
    status === Estate.ACTIVE ? "Activo" : "Inactivo";
                ...
                >
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setEditingRecipe({
                        ...editingRecipe,
                        recipeStatus: e.target.value === "ACTIVE" ? Estate.ACTIVE : Estate.INACTIVE,
                      })
                    }
                    className="border rounded px-4 py-2 w-full"
                  >
        ...
        onSubmit={async (e) => {
          ...
          try {
            ...
            if (startHour && endHour) {
              res = await axios.get(`http://localhost:8080/api/recipes/movementByRangeHour`, {
                params: {
                  date,
              ...
              res = await axios.get(`http://localhost:8080/api/recipes/movementByDate`, {
                params: { date },
              });
            }
